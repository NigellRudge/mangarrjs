...
import {
  mergeGenresResults,
  mergeSourceResults,
} from "@mangarr/shared/types/dtos/shared";
...
export default class SearchService {
  ...
  public async search(
    query: string,
    filters: { page?: number; pageSize?: number } = { page: 1, pageSize: 25 },
  ): Promise<MangaResponse[]> {
    const redisKey = `search-${JSON.stringify({ query, filters })}`;
  ...
  private async getGenreFacets(sources: MangaSourceType[]) {
    ...
    return mergeGenresResults(resultsMap);
  }
}
